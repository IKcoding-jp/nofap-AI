...
## 3. モテ（内部指標と表示）
ストリーク日数に加えて、日々の自分磨き（筋トレ、習慣など）を総合的に評価する指標。\n+内部では細かい変化を扱い、UIでは分かりやすく前向きなスケールで表示する。

### 3.1 構成要素（属性）
以下の4つの属性の平均値を「内部モテレベル」とする。
...
### 3.2 算出式（内部モテレベル）
`Mote Level = (Confidence + Vitality + Calmness + Cleanliness) / 4`\n+※各属性は-100から100ポイントの範囲。内部モテレベルも-100から100の範囲にクランプされる。

### 3.3 表示用のモテスコア（0〜100）
UIでは「モテスコア（0〜100）」を表示し、ユーザーが直感的に成長を感じられるようにする。\n+\n+- 変換式: `Mote Score = round((Mote Level + 100) / 2)`\n+- 例: `-100 → 0`, `0 → 50`, `100 → 100`\n+\n+ランク表示（少数段階）もこのモテスコアに基づいて行う（例: 10段階）。

### 3.4 ステージ定義（200段階 / 内部の細かな味付け）

内部モテレベル-100から100までの各レベルに固有のステージ名称が設定されています。詳細な一覧は `lib/mote-stages.ts` を参照してください。
...
- リセット時：`Current Attribute = Current Attribute * 0.7`（モチベ維持のため、落ちすぎない調整）
