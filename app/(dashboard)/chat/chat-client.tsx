...
export default function ChatClient({ initialSessionId, initialMessages }: { initialSessionId?: number; initialMessages: any[] }) {
  ...
  // ホーム画面からのペンディングメッセージを処理
  const pendingTriggered = useRef(false);
  useEffect(() => {
    if (searchParams.get("pending") === "true" && !pendingTriggered.current && !isLoading && currentSessionId) {
      const pendingMessage = localStorage.getItem("pendingChatMessage");
      if (pendingMessage) {
        pendingTriggered.current = true;
        localStorage.removeItem("pendingChatMessage");
        if (sendMessage) {
          sendMessage({ text: pendingMessage });
        }
        // URLからpendingパラメータを削除
        router.replace(`/chat?session=${currentSessionId}`);
      }
    }
  }, [searchParams, isLoading, currentSessionId, sendMessage, router]);
